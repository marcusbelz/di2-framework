# Bug Index

<!-- Auto-gepflegt durch /bug. Pro Bug eine eigene Datei in docs/bug/. Status-Werte: ❌ Offen / ✅ Behoben / 🚫 Won't Fix. -->

## Status

| Feld | Wert |
|------|------|
| Zuletzt aktualisiert | 2026-06-11 |
| Offene Bugs | 0 |
| Behobene Bugs | 2 |
| Won't Fix | 0 |

## Bug-Historie

> Sortierung: ID **absteigend** (neueste oben). Eine Zeile pro Bug. Link führt direkt zur Bug-Datei.

| ID | Titel | Erfasst am | Geschlossen am | Geschlossen-Hinweis | Quelle |
|----|-------|------------|----------------|---------------------|--------|
| [BUG-0002](archive/2026-Q2/bug-0002-sqlfluff-pg01-fehlalarm-create-index.md) | sqlfluff PG01-Fehlalarm auf konventionellem CREATE INDEX | 2026-06-11 | 2026-06-11 | ✅ Behoben (PG01 in .sqlfluff exclude_rules ergänzt) | qa |
| [BUG-0001](archive/2026-Q2/bug-0001-umgebungs-reihenfolge-int-test-vertauscht.md) | Branch→Umgebung-Zuordnung: int und test vertauscht | 2026-06-09 | 2026-06-09 | ✅ Behoben (dev/int ← dev, test/prod ← main; Env + Doku) | manual |
