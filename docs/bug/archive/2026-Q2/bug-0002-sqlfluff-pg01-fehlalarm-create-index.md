...
**Lösung:**
- **Root Cause (bestätigt):** `.sqlfluff` `exclude_rules` listete `PG01` (`postgres.excessive_locks`) nicht. Die Regel verlangt `CREATE INDEX CONCURRENTLY`, was die Hauskonvention (`sql.md`: `CREATE [UNIQUE] INDEX IF NOT EXISTS …` ohne CONCURRENTLY) bewusst nicht nutzt und im transaktionalen `psql -f`-Deploy ohnehin nicht erlaubt ist — also ein Fehlalarm im Sinne von AC 5.
- **Geänderte Dateien:**
  - [.sqlfluff](../../../.sqlfluff): `postgres.excessive_locks` zu `exclude_rules` ergänzt, Kommentarblock um die Begründung erweitert (Commit `1502212`).
- **Lösungsschritte:**
  1. `postgres.excessive_locks` (PG01) in `exclude_rules` aufgenommen — analog zu `layout` / `references.keywords` / `capitalisation.identifiers`.
  2. Kommentar in `.sqlfluff` um die PG01-Begründung ergänzt (Verweis BUG-0002).
- **Warum das funktioniert:** Konventionelle `CREATE INDEX`-DDL löst keinen Lint-Fehler mehr aus (AC 5 erfüllt); Parse-/Syntax-Checks bleiben aktiv. `/qa`-Re-Test (2026-06-11) gegen den committeten HEAD (mit `CREATE INDEX` auf `execution`): sqlfluff grün (31 Dateien), shellcheck grün, kaputtes SQL weiterhin rot.
