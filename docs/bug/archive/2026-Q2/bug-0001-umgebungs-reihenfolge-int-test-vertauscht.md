...
**Lösung:**
- **Root Cause (bestätigt):** In di2f-0002 war die Zuordnung dev+test ← dev und int+prod ← main — nicht monoton zur Reihenfolge dev < int < test < prod, dadurch wirkten `int`/`test` vertauscht.
- **Geänderte Dateien:**
  - GitHub-Environments (extern): `int` Deployment-Branch `main`→`dev`, `test` `dev`→`main` (manuell durch User; per GitHub-API verifiziert).
  - [features/di2f-0004-github-actions-db-workflows-secrets.md](../../../features/di2f-0004-github-actions-db-workflows-secrets.md): Mapping-Texte auf `dev`/`int` ← `dev`, `test`/`prod` ← `main`.
  - [.github/workflows/db-create.yml](../../../.github/workflows/db-create.yml), [db-deploy.yml](../../../.github/workflows/db-deploy.yml), [README.md](../../../.github/workflows/README.md): Kopf-Kommentare/Beschreibung.
  - [.claude/skills/help/SKILL.md](../../../.claude/skills/help/SKILL.md): zwei Stellen.
  - [features/archive/di2f-0002-git-branch-und-deploy-strategie.md](../../../features/archive/di2f-0002-git-branch-und-deploy-strategie.md): Korrektur-Hinweis oben (Historie/QA bewusst unverändert).
- **Lösungsschritte:**
  1. Operative Doku/Workflow-Texte auf `dev`/`int` ← `dev`, `test`/`prod` ← `main` umgestellt (Commit `962f8b9`).
  2. di2f-0002 (archiviert/Deployed): Korrektur-Note statt History-Rewrite (Audit-Trail).
  3. GitHub-Environment-Deployment-Branches `int` (→`dev`) und `test` (→`main`) manuell getauscht.
  4. QA-Re-Test: per GitHub-API verifiziert (dev→dev, int→dev, test→main, prod→main); keine operativen Alt-Treffer mehr.
- **Warum das funktioniert:** Mit `dev`/`int` ← `dev` und `test`/`prod` ← `main` entspricht die Branch-Aufteilung der monotonen Reihenfolge dev → int → test → prod (konsistent zum Parallelprojekt). Workflows funktional unberührt (`github.ref_name` + native Env-Policy).
