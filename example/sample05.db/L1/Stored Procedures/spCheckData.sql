...
--          Created
-- --------------------------------------------------------------------------------
CREATE procedure [L1].[spCheckData] 
(   
    @p_executionId           AS int
   ,@p_schemaName            AS nvarchar(128)
   ,@p_tableName             AS nvarchar(128)
         ...
         BEGIN
            ...
            IF LOWER(@c_procedure) = LOWER(N'spInsertErrorCheckConstraint')
               BEGIN
                  ...
                  IF @errortype IS NOT NULL
                     BEGIN
                        SET @constraint = @c_constraint + ' AND [ExecutionID] = ' + CAST(@p_executionId AS nvarchar(100));
                        ...
                        EXEC [LOG].[spInsertErrorCheckConstraint] @p_executionId, @componentId, @traceId, @errortype, @component, @task, @entity, @step, @schema, @table, @id1_field, @id2_field, @id3_field, @c_check_Field, @message, @affectedrows OUTPUT, @constraint;
            ...
            ELSE IF LOWER(@c_procedure) = LOWER(N'spInsertErrorCheckUniqueIdColumns')
               BEGIN
                  ...
                  IF @errortype IS NOT NULL
                     BEGIN
                        SET @whereClause = @whereClause + ' AND [ExecutionID] = ' + CAST(@p_executionId AS nvarchar(100));
                        EXEC [LOG].[spInsertTrace] @p_executionId, @componentId, @traceId_loop OUTPUT, @source, @component, @task, @entity, @step, @description, 'Check', NULL, 'processing', 0;
                        EXEC [LOG].[spInsertErrorCheckUniqueIdColumns] @p_executionId, @componentId, @traceId, @errortype, @component, @task, @entity, @step, @schema, @table, @id1_field, @id2_field, @id3_field, @c_check_Field, @maxOccurance, @message, @affectedrows OUTPUT, @whereClause;
                        EXEC [LOG].[spUpdateTraceSuccess] @traceId_loop, NULL, 'Check', @affectedrows;
                     END;
               END;
...
-- EXEC [L1].[spCheckData]  148, N'L1', N'lead', N'L1',