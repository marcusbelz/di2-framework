...
-- Description: Checks whether the file with the file name passed in the parameter 
--              p_fileName does exist.
-- 
...
--          Created
-- --------------------------------------------------------------------------------
CREATE FUNCTION [dbo].[fnFileExists] (@p_fileName AS nvarchar(1024))
RETURNS bit
AS
BEGIN
   ...
   DECLARE @fileExists        varchar(20);
   DECLARE @result            bit;

   SET @result = 0;
   SET @p_fileName = LTRIM(RTRIM(@p_fileName));
   ...
   ELSE
      BEGIN
         ...
         -- ----------------------------------------------------------------------
         -- Call function 'FileExists'
         -- ----------------------------------------------------------------------
         EXECUTE @oleResult = sp_OAMethod @fs, 'FileExists', @fileExists OUT, @p_fileName;
         --IF (@oleResult <> 0) 
         --   BEGIN
         --      EXECUTE sp_OAGetErrorInfo @error_object, @error_source OUTPUT, @error_description OUTPUT, @error_helpfile OUTPUT,@error_helpid OUTPUT;
         --      SET @error_description = COALESCE(@error_description,'<no description available>', @error_description);
         ...
         SET @result = CASE WHEN @fileExists = 'True' THEN 1 ELSE 0 END;
      END;
