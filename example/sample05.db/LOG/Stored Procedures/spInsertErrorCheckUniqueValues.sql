...
-- Description: Checks the uniqueness of values in the column 
--              @p_schema.@p_table.@p_checkField. Values that 
...
--          Created
-- --------------------------------------------------------------------------------
CREATE PROCEDURE [LOG].[spInsertErrorCheckUniqueValues]
    @p_executionId           AS int
   ,@p_componentId           AS int
   ,@p_traceId               AS int
   ,@p_errorType             AS nchar(1)
...
BEGIN
   ...
   BEGIN
      ...
      -- Check @p_checkField 
      -- --------------------------------------------------------------------------------
      IF (@p_checkField  IS NULL)
         BEGIN
            SET @message = 'The parameter ''p_check_field'' is NULL.';
            EXEC [dbo].[spRaiseError] @message,  @component
            RETURN -1;
         END;
      -- --------------------------------------------------------------------------------
   ...
   SET @sql = @sql + '      ,COUNT([' + @p_checkField  + ']) AS [COUNT] '                                       + CHAR(13);
   ...
   SET @sql = @sql + '      COUNT([' + @p_checkField  + ']) > ' + CAST(@p_maxOccurance AS nvarchar(100)) + ' '  + CHAR(13);
   SET @sql = @sql + ') '                                                                                       + CHAR(13);
   ...
   SET @sql = @sql + '   , CAST([T02].' + @p_ID1Field + ' AS nvarchar(128))         AS [ID1Value] '            + CHAR(13);
   ...
   ELSE
      BEGIN
         SET @sql = @sql + '   , CAST([T02].' + @p_ID2Field + ' AS nvarchar(128))   AS [ID2Value] '            + CHAR(13);
   ...
   ELSE
      BEGIN
         SET @sql = @sql + '   , CAST([T02].' + @p_ID3Field + ' AS nvarchar(128))   AS [ID3Value] '            + CHAR(13);
   ...
   SET @sql = @sql + '      [T01].' + @p_checkField  + ' = [T02].' + @p_checkField  + '; '                      + CHAR(13);
   ...
   RETURN 0;
END
-- [LOG].[spInsertErrorCheckUniqueValues]
...
--DECLARE @RC                AS int;
--DELCARE @p_executionId     AS int;
--DELCARE @p_componentId     AS int;
--DELCARE @p_traceId         AS int;
--DELCARE @p_errorType       AS nchar(1);
--DELCARE @p_component       AS nvarchar(128);
--DELCARE @p_task            AS nvarchar(128);
--DELCARE @p_entity          AS nvarchar(128);
--DELCARE @p_step            AS nvarchar(max);
--DELCARE @p_schema          AS nvarchar(128);
--DELCARE @p_table           AS nvarchar(128);
--DELCARE @p_ID1Field        AS nvarchar(128);
--DELCARE @p_ID2Field        AS nvarchar(128);
--DELCARE @p_ID3Field        AS nvarchar(128);
--DELCARE @p_checkField      AS nvarchar(1000);
--DELCARE @p_maxOccurance    AS int;
--DELCARE @p_message         AS nvarchar(max);
--DELCARE @p_affectedRows    AS int OUT;
--DELCARE @p_whereClause     AS nvarchar(max);
...
--SET @p_message      = 'Too Many Occurances'
--SET @p_whereClause  = NULL;

--EXECUTE @RC = [LOG].[spInsertErrorCheckUniqueValues]
-- @p_executionId           AS int
--,@p_componentId           AS int
--,@p_traceId               AS int
--,@p_errorType             AS nchar(1)
