...
-- Description: Inserts a row in table [LOG].[Trace] with the [State] = 'processing' 
--              and [Success] = '0'.
--
...
--          Created
-- --------------------------------------------------------------------------------
CREATE PROCEDURE [LOG].[spInsertTraceProcessing]
...
BEGIN
   ...
   EXEC [LOG].[spInsertTrace]
      ...
      ,'start'           -- @p_step           nvarchar(max)
      ,@p_description    -- @p_description    nvarchar(max) > User::traceDescription
      ,NULL              -- @p_action         nvarchar(100)
      ,0                 -- @p_affectedRows   int
      ,'processing'      -- @p_state          nvarchar(100) 
      ,0;                -- @p_success        bit
END;
-- [LOG].[spInsertTraceProcessing]

-- EXEC [LOG].[spInsertTraceProcessing] 1, 22, 54, 'T1_001_PackageName', 'SQL 0100 Do Somethingh', 'Contacts', 'Insert new Contacts'
