...
--              will be passed in the parameter p_length. Texts that 
--              are too long will be logged in the table [LOG].[Error].
-- --------------------------------------------------------------------------------
-- Parameters
...
--          Created
-- --------------------------------------------------------------------------------
CREATE PROCEDURE [LOG].[spInsertErrorCheckNVarCharLength1]
    @p_executionId           AS int
   ,@p_componentId           AS int
   ,@p_traceId               AS int
   ,@p_errorType             AS nchar(1)
...
BEGIN
   ...
   BEGIN
      ...
      IF [dbo].[fnIsNullOrEmpty](@p_checkField , 1) = 1
         BEGIN
            SET @message = 'The parameter ''p_error_field'' is NULL or empty.';
            EXEC [dbo].[spRaiseError] @message,  @component
            RETURN -1;
         END;
   END;
   ...
   SET @sql = @sql + '   ,''' + @p_message + ' ('' + CAST((DATALENGTH([T01].[' + @p_checkField  + ']) / 2) AS nvarchar(100)) + '' / ' + CAST(@p_length as nvarchar(100)) + ')'' AS [Description] ' + CHAR(13);
   ...
   SET @sql = @sql + '       DATALENGTH([T01].[' + @p_checkField  + ']) / 2 > ' + CAST(@p_length as nvarchar(100)) + ' ' + CHAR(13);
   ...
   RETURN 0;
END
-- [LOG].[spInsertErrorCheckNVarCharLength1]
...
--SET @p_whereClause = 'ID > 1000';

--EXECUTE @RC = [LOG].[spInsertErrorCheckNVarCharLength1]
--   @p_executionId
--  ,@p_componentId
--  ,@p_traceId
--  ,@p_errorType
